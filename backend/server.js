...
app.use((req, res, next) => {
  ...
  res.end = function (chunk, ...args) {
    ...
    if (res.statusCode >= 400) {
      ...
      logger.info(`(Response Sent from Shutterstock-Replicate Service) Trace ID: ${req.traceId} , Status Code: ${res.statusCode}, Response Body: ${body}`);
    }
    else {
      logger.info(`(Response Sent from Shutterstock-Replicate Service) Trace ID: ${req.traceId} , Status Code: ${res.statusCode}, Status Message: ${getReasonPhrase(res.statusCode)}`);
    }
    originalEnd.apply(res, [chunk, ...args]);
  };
